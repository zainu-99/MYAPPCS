...
namespace MYAPPCS
{
    public partial class FormErrorReport : Form
    {
        ...
        {
            String query = "select Top " + ComboBoxLimit.Text + " a.Id,a.Action,a.MsgError,a.Datetime,a.Note,concat(b.userid,'-',b.nama) as [User] from ErrorReport a left join Users b on a.UserId =b.ID where (convert(date,[Datetime])>='" + DateTimePickerStart.Value.ToString("yyyy-MM-dd") + "' and convert(date,[Datetime]) <= '" + DateTimePickerUntil.Value.ToString("yyyy-MM-dd") + "') and (a.Action like '%" + txtSearch + "%' or a.MsgError like '%" + txtSearch + "%'  or a.Note like '%" + txtSearch + "%' or b.nama like '%" + txtSearch + "%' ) order by a.Id desc";
